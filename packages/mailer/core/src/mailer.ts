    ...
    if (testHandlerKey === undefined) {
      ...
      try {
        this.fallbackTestHandler =
          new (require('@redmix/mailer-handler-in-memory').InMemoryMailHandler)()
        this.logger.warn(
          "Automatically loaded the '@redmix/mailer-handler-in-memory' handler, this will be used to process mail in test mode",
        )
      } catch {
        this.logger.warn(
          "No test handler specified and could not load the '@redmix/mailer-handler-in-memory' handler automatically, this will prevent mail from being processed in test mode",
        )
      }
    ...
    if (developmentHandlerKey === undefined) {
      ...
      try {
        this.fallbackDevelopmentHandler =
          new (require('@redmix/mailer-handler-studio').StudioMailHandler)()
        this.logger.warn(
          "Automatically loaded the '@redmix/mailer-handler-studio' handler, this will be used to process mail in development mode",
        )
      } catch {
        this.logger.warn(
          "No development handler specified and could not load the '@redmix/mailer-handler-studio' handler automatically, this will prevent mail from being processed in development mode",
        )
      }
