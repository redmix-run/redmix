...
async function importModule(
  ...
  if (isDev) {
    if (mod === 'rd-server') {
      ...
      const loadedMod = await import('@redmix/router/location')
      return loadedMod
    } else if (mod === '__rwjs__server_auth_provider') {
      const loadedMod = await import(
        '@redmix/auth/dist/AuthProvider/ServerAuthProvider.js'
      )
      return loadedMod
    } else if (mod === '__rwjs__server_inject') {
      const loadedMod = await import('@redmix/web/serverInject')
      return loadedMod
    }
  } else {
