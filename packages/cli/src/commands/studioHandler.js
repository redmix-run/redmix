...
export const handler = async (options) => {
  try {
    ...
    if (!isModuleInstalled('@redmix/studio')) {
      ...
      await installModule('@redmix/studio', '12')
      console.log('Studio package installed successfully.')

      const installedRealtime = await installModule('@redmix/realtime')
      if (installedRealtime) {
        console.log(
          "Added @redmix/realtime to your project, as it's used by Studio",
        )
      }

      const installedApiServer = await installModule('@redmix/api-server')
      if (installedApiServer) {
        console.log(
          "Added @redmix/api-server to your project, as it's used by Studio",
        )
      }
    }

    // Import studio and start it
    const { serve } = await import('@redmix/studio')
...
function getProjectRedwoodVersion() {
  ...
  return devDependencies['@redmix/core']
}
