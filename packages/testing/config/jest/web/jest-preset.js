...
module.exports = {
  ...
  moduleNameMapper: {
    ...
    // We replace imports to "@redmix/router" with our own "mock" implementation.
    '^@redmix/router$': path.join(
      NODE_MODULES_PATH,
      '@redmix/testing/dist/web/MockRouter.js',
    ),
    '^@redmix/web$': path.join(NODE_MODULES_PATH, '@redmix/web/dist/cjs'),

    // This allows us to mock `createAuthentication` which is used by auth
    // clients, which in turn lets us mock `useAuth` in tests
    '^@redmix/auth$': path.join(
      NODE_MODULES_PATH,
      '@redmix/testing/dist/web/mockAuth.js',
    ),

    // @NOTE: Import @redmix/testing in web tests, and it automatically remaps to the web side only
    // This is to prevent web stuff leaking into api, and vice versa
    '^@redmix/testing$': path.join(NODE_MODULES_PATH, '@redmix/testing/web'),
    ...
    '\\.(jpg|jpeg|png|gif|eot|otf|webp|svg|ttf|woff|woff2|mp4|webm|wav|mp3|m4a|aac|oga|css)$':
      '@redmix/testing/dist/web/fileMock.js',
  },
  transform: {
    '\\.[jt]sx?$': [
      'babel-jest',
