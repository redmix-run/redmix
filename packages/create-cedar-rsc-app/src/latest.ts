...
export function relaunchOnLatest(config: Config) {
  ...
  if (config.verbose) {
    if (process.argv.includes('--npx')) {
      ...
      console.log(
        'cmd:',
        'npx',
        ['create-cedar-rsc-app@latest', ...args].join(' '),
      )
    }
  }
  ...
  if (process.argv.includes('--npx')) {
    ...
    result = spawnSync(
      'npx',
      ['create-cedar-rsc-app@latest', ...args],
      spawnOpts,
    )
  }
  ...
  if (result.error) {
    console.error(
      'There was an error launching the latest version of create-cedar-rsc-app.',
    )
    console.error('Please try running it manually with `@latest')
    console.error('npx -y create-cedar-rsc-app@latest APP_PATH')
