...
- Copy [`packages/create-cedar-app/template/api/src/lib/db.ts`](https://github.com/cedarjs/cedar/blob/main/packages/create-cedar-app/template/api/src/lib/db.ts) and replace `api/src/lib/db.ts` (or `api/src/lib/db.js`). _Optional_.
 ...
 */
import { redactionsList } from '@cedarjs/api/logger'
...
Both `emitLogLevels` and `handlePrismaLogging` are `@cedarjs/api/logger` package exports.
