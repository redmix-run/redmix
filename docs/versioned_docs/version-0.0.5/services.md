...
You'll import the three functions below from `@cedarjs/api`:

```jsx
import { validate, validateWith, validateUniqueness } from '@cedarjs/api'
```
...
```js title="api/src/lib/cache.js"
import { createCache, MemcachedClient } from '@cedarjs/api/cache'
