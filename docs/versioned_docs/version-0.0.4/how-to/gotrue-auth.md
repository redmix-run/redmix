...
Then we'll need to import `useAuth` from `@cedarjs/auth` and destructure `logIn` so that we can use it in our `onSubmit` function:
...
Now we can import [`useAuth` from `@cedarjs/auth`](../authentication.md#api). We'll destructure its `logOut` method and invoke it in the `onClick` function:
...
> If you spot an error or have trouble completing any part of this recipe, please feel free to open an issue on [Github](https://github.com/cedarjs/cedar) or create a topic on our [community forum](https://community.redwoodjs.com/).
