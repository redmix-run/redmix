...
if (hasCRWA_OkLabel) {
  ...
  const didRebuildJS_Template = changedFiles.some((file) =>
    file.startsWith('packages/create-cedar-app/templates/js'),
  )
  ...
  if (didRebuildJS_Template) {
    ...
    const shouldRebuildJS_Template = changedFiles.some((file) =>
      file.startsWith('packages/create-cedar-app/templates/ts'),
    )
    ...
    if (!shouldRebuildJS_Template) {
      ...
      console.log(
        [
          '',
          'This PR changes the create-cedar-app TS template.',
